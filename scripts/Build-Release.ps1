...
$TranslationsStage = Join-Path $StageRoot "translations"
$Zip = Join-Path $Dist "EsotericEbb-FR-Static-$Version.zip"

if (-not $SkipBuild) {
  dotnet build $PatcherProject -c $Configuration
}
...
Copy-Item $TranslationsSource $TranslationsStage -Recurse -Force
